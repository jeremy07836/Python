...
def parse_invoice_number(invoice_number: str) -> tuple[int, int]:
    ...
    invoice_string = invoice_number.split('-')
    invoice = int(invoice_string[0]), int(invoice_string[1])
...
def next_invoice_number(invoice_number: str) -> str:
    ...
    # year = get_year()
    next_invoice = str(get_year()) + "-"
    invoice = parse_invoice_number(invoice_number)
    if invoice[0] != get_year():
        next_invoice += f'{1:04d}'
    else:
        next_invoice += f"{(invoice[1] + 1):04d}"
    return next_invoice
...
def record_invoice(invoice_file: TextIO,
    ...
    """
    # invoice_file.seek(last_line_ptr, 0)   # SEEK_SET = 0
